...
import { addAssociationProject, getAssociationProject, updateAssociationProject, findAssociationProject } from "../services/associationProjectService"
...
export const useAssociationProject = ( profileCod ) => {
            ...
            validations: [
                (value, allValues) => {
                    ...
                    if (selectedDate < startProject || selectedDate > endProject) {
                        return `La fecha de participación debe estar dentro del rango del proyecto (desde ${formatDateDDMMYYYY(project.project_start_date)} hasta ${formatDateDDMMYYYY(project.project_end_date)}).`;
                    }
                    return null;
                }
            ]
    ...
    */
    ...
    const [searchFeature, setSearchFeature] = useState(fieldsAssociation[0]?.name || "");

    /*
    * Manejo de historial para evitar busqueda con la paginacion
    */
    const [appliedField, setAppliedField] = useState(fieldsAssociation[1]?.name || "");
    ...
    */
    const fetchAssociation = async (pageNum = page, projectsData = projects) => {
        try {
            ...
            const resp = await getAssociationProject(profileCod.profileCod, pageNum, pageSize);
            const enriched = resp.data.data.map(assoc => {
                const project = projectsData.find(p => p.cod_project === assoc.cod_project);
                return {
                    ...assoc,
    ...
    */
    ...
    const handleSearch = async () => {
        try {
            ...
            const resp = await findAssociationProject(1, pageSize, searchFeature, searchText, profileCod.profileCod);

            const enriched = resp.data.data.map(assoc => {
                const project = projects.find(p => p.cod_project === assoc.cod_project);
                return {
                    ...assoc,
                    project_name: project?.project_name || "Proyecto no encontrado",
                    project_company: project?.project_company || "Empresa no disponible",
    ...
    */
    const findAndSetFiltered = async (pageNum = 1) => {
        try {
            setLoadingAssociation(true);
            const resp = await findAssociationProject(pageNum, pageSize, appliedField, appliedText, profileCod.profileCod);
            const enriched = resp.data.data.map(assoc => {
                const project = projects.find(p => p.cod_project === assoc.cod_project);
                return {
                    ...assoc,
