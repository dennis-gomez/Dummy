...
import { CircularProgress, Stack, Pagination, PaginationItem, Box } from "@mui/material";
...
const ResumeTableLicitationTable = ({
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody>
              {items.map((row, idx) => (
                ...
                >
                  {columns.map((col) => (
                    <td key={col.key} className="py-2 px-4 text-center align-middle">
                      {col.type === "date"
  ...
  // 🔹 Estados para los selectores de búsqueda
  const [primaryValue, setPrimaryValue] = useState("");
  const [secondaryValue, setSecondaryValue] = useState("");
  const [dynamicValue, setDynamicValue] = useState("");


  // 🔹 Secondary fields según el contexto seleccionado
  const secondaryFields = useMemo(() => {
    const context = contextOptions.find(c => c.value === primaryValue);
    return context?.options || [];
  }, [primaryValue, contextOptions]);

  // 🔹 Dynamic options según el campo secundario seleccionado
  const dynamicOptions = useMemo(() => {
    if (!secondaryValue) return [];
    const field = secondaryFields.find(f => f.value === secondaryValue);
    return field?.options || [];
  }, [secondaryValue, secondaryFields]);
  ...
  const handlePrimaryChange = (value) => {
    ...
    setSecondaryValue(""); // limpiar secundario
    setDynamicValue("");   // limpiar dinámico
  };

  // Cuando cambia el valor secundario
    ...
    <div className="p-6 mt-6 bg-white rounded-2xl shadow-lg">
      <div className="flex flex-col lg:flex-row gap-4 w-full max-w-5xl mx-auto mb-4">
        <Box className="flex flex-wrap gap-3 bg-white rounded-xl p-4 flex-1">
          ...
          <DoubleSeeker
            primaryOptions={contextOptions}
            secondaryFields={secondaryFields}
            primaryLabel="Modelo de búsqueda"
            secondaryLabel="Filtrar por"
            dynamicLabel="Valor"
            primaryValue={primaryValue}
            setPrimaryValue={handlePrimaryChange}   // usa la función
            secondaryValue={secondaryValue}
            setSecondaryValue={handleSecondaryChange} // usa la función
          ...
          <table className="min-w-full border-collapse">
            ...
            <thead>
              <tr className="bg-gradient-to-r from-blue-600 to-blue-500 text-white">
                ...
                <th className="py-4 px-6 text-center font-semibold text-md capitalize tracking-wider rounded-tr-xl"> Perfiles </th>
              </tr>
            </thead>
                ...
                <Fragment key={person.personal_cod}>
                  ...
                  <tr className="hover:bg-gray-100 transition-colors">
                    ...
                    <td className="text-center">
                      <button onClick={() => onExpand(person.personal_cod)} className="p-2 rounded hover:bg-gray-200">
                    ...
                    {fields.map((f) => (
                      <td key={f.name} className="py-4 px-6 text-center align-middle text-gray-700">
                        {f.type === "date" ? formatDateDDMMYYYY(person[f.name])
                          : f.type === "select" ? f.options?.find((opt) => String(opt.value) === String(person[f.name]))?.label || ""
                            : person[f.name] || ""}
                      </td>
                    ))}
                    ...
                    <td className="py-3 px-4 text-center">
                      <select onChange={(e) => handleProfileSelect(person.personal_cod, e.target.value)}
